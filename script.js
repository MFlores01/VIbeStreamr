...
// Function to load a VRM model from a file
    ...
    loader.parse(arrayBuffer, '', (gltf) => {
      ...
      THREE.VRM.from(gltf).then((vrm) => {
        if (currentVrm) {
          ...
          currentVrm.dispose(); // Clean up old VRM model
        }
        scene.add(vrm.scene);
        currentVrm = vrm;
...
// Handle upload button and file input
...
loader.load(
  "GuelGamesh1_.vrm", // Replace with your correct relative path
  ...
  gltf => {
    ...
    THREE.VRM.from(gltf).then(vrm => {
      scene.add(vrm.scene);
      currentVrm = vrm;
      currentVrm.scene.rotation.y = Math.PI; // Rotate model 180deg to face the camera
    });
  },
...
// Using `Mediapipe` utils to get camera - lower resolution = higher fps
